...
└── src/
    ...
    │
    ├── lib/
    │   └── constantes.ts          ← Configuración de estados, colores, fechas
    │
    ├── api/
    │   └── seguirOrden.ts         ← Llama a la Edge Function de Supabase
    │
    ├── hooks/
    │   └── useSeguimientoOrden.ts      ← Hook: maneja loading / error / data
    │
    ├── pages/
    │   └── PaginaSeguimiento.tsx         ← Página principal /track/:codigo
    │
    └── components/
        ├── EsqueletoCarga.tsx   ← Skeleton animado mientras carga
        ├── EstadoError.tsx        ← Pantalla de error amigable
        ├── EncabezadoOrden.tsx       ← Cabecera: código, badge de estado, vehículo
        ├── ProgresoOrden.tsx     ← Barra de progreso con 5 pasos
        ├── GaleriaMedia.tsx      ← Galería con tabs ANTES/PROCESO/DESPUÉS
        ├── LineaTiempo.tsx          ← Historial de actividad
        └── NotasPublicas.tsx       ← Notas del taller
```
...
```
→ React Router lo dirige a **`PaginaSeguimiento.tsx`**

### 2. PaginaSeguimiento extrae los parámetros
```typescript
// src/paginas/PaginaSeguimiento.tsx
const { codigo } = useParams();           // "SC-0001"
const token = searchParams.get('token');   // "b3db9aa9..."
```

### 3. El hook `useSeguimientoOrden` llama a la API
```typescript
// src/ganchos/useSeguimientoOrden.ts
const { data, loading, error, refetch } = useSeguimientoOrden({ codigo, token });
```

### 4. La función `fetchSeguimientoOrden` hace el fetch real
```typescript
// src/servicios/seguirOrden.ts
...
### 6. PaginaSeguimiento renderiza los componentes
```
data.order      → EncabezadoOrden + ProgresoOrden
data.media      → GaleriaMedia (usa signed_url para cargar las fotos)
data.timeline   → LineaTiempo
data.order.notas_publicas → NotasPublicas
```
...
### Colores de estados (`src/biblioteca/constantes.ts`)
| Estado | Color | Paso |
|---|---|---|
...
Edita `src/biblioteca/constantes.ts` y agrega el nuevo estado al objeto `STATUS_CONFIG`:
...
Por eso el componente `GaleriaMedia` usa `loading="lazy"` — si la URL expira y el usuario recarga, el hook `refetch` genera nuevas URLs.
...
| Agregar colores/estados | `src/biblioteca/constantes.ts` |
| Agregar componentes | `src/componentes/` |
