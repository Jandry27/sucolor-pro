...
vi.mock('@/hooks/useSearchOrder', () => ({
    useSearchOrder: vi.fn(() => ({
        loading: false,
        error: null,
        search: mockSearch,
        reset: vi.fn(),
...
// ─── Helper ───────────────────────────────────────────────────────────────────
const renderForm = () => render(<MemoryRouter><SearchForm /></MemoryRouter>);
...
describe('SearchForm', () => {
    ...
    it('muestra el mensaje de error cuando el hook reporta un error', async () => {
        const { useSearchOrder } = await import('@/hooks/useSearchOrder');
        (useSearchOrder as ReturnType<typeof vi.fn>).mockReturnValue({
            loading: false,
