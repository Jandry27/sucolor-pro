        ...
        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
                <div>
                    ...
                    {order.cliente && (
                        <p className="mt-1 text-sm text-[rgba(11,18,32,0.55)] dark:text-slate-400">
                            {order.cliente}
                        </p>
                    )}
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
                ...
                {order.fecha_estimada && (
                    <DateRow
                        ...
                        value={formatDate(order.fecha_estimada)}
                        highlight
                    />
                )}
            </div>
    ...
    return (
        <div>
            ...
            <div className="flex items-center gap-1.5">
                {prefix}
                <p
                    className={`text-sm font-semibold text-[#0F172A] dark:text-slate-200 ${mono ? 'font-mono-code' : ''}`}
                >
                    {value || '—'}
                </p>
            </div>
...
function DateRow({
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <div>
                <p className="text-xs text-[rgba(11,18,32,0.40)] dark:text-slate-400 mb-0.5">
                    {label}
                </p>
                <p
                    className={`text-sm font-semibold ${highlight ? 'text-[#F97316] dark:text-[#FB923C]' : 'text-[#0F172A] dark:text-slate-200'}`}
                >
                    {value || 'Por confirmar'}
                </p>
            </div>
