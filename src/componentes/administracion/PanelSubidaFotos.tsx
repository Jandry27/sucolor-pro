...
interface PanelSubidaFotosProps {
    ordenId: string;
    ordCodigo: string;
}
...
export function PanelSubidaFotos({ ordenId }: PanelSubidaFotosProps) {
    ...
    const loadMedia = useCallback(async () => {
        ...
        const { data } = await supabase
            .from('media')
            .select('id, url, categoria, caption, storage_path, storage_bucket')
            .eq('orden_id', ordenId)
    ...
    const handleFiles = async (files: FileList | null) => {
        ...
        for (const file of Array.from(files)) {
            try {
                ...
                if (insertErr)
                    throw new Error(`Foto subida pero no guardada en DB: ${insertErr.message}`);

                setMedia(prev => [
                    ...prev,
                    {
                        id: mediaRow?.id ?? `temp_${Date.now()}`,
                        url: secureUrl,
                        categoria: activeTab,
                    },
                ]);
    ...
    return (
        <div className="glass-card space-y-4" style={{ padding: '20px' }}>
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="ml-auto text-xs text-[rgba(15,23,42,0.40)]">
            ...
            <div className="segmented-control">
                {CATS.map(c => (
                    <button
                        key={c.key}
                        className={`seg-btn flex items-center gap-1 ${activeTab === c.key ? 'active' : ''}`}
