...
import { formatDateTime } from '@/biblioteca/constantes';
...
const Lightbox = ({ src, onClose }: { src: string; onClose: () => void }) => (
    <AnimatePresence>
        ...
        >
            <div
                className="w-full max-w-3xl flex justify-between items-center mb-4 z-10"
            ...
            >
                <a
                    ...
                    className="bg-white/10 border border-white/10 hover:bg-white/20 text-white backdrop-blur-md px-3 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5"
                >
...
interface PanelGastosPublicoProps {
    gastos: OrdenGasto[];
}

export function PanelGastosPublico({ gastos }: PanelGastosPublicoProps) {
        ...
        >
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2 text-[#F97316] dark:text-[#FB923C]">
                    ...
                    <h3 className="font-semibold text-[#0F172A] dark:text-slate-100 text-sm">
                        Repuestos y Gastos
                    </h3>
                </div>
                {totalGastos > 0 && (
            ...
            {gastos && gastos.length > 0 ? (
                    ...
                    {gastos.map((gasto, i) => (
                        ...
                        >
                            <div className="flex items-start sm:items-center gap-4">
                                ...
                                <div>
                                    <p className="font-medium text-sm text-[rgba(11,18,32,0.9)] dark:text-slate-200">
                                        {gasto.descripcion}
                                    </p>
                                    <p className="text-xs text-[rgba(11,18,32,0.4)] dark:text-slate-500 mt-0.5">
                ...
                <div className="flex flex-col items-center justify-center py-10 px-4 text-center rounded-xl border border-dashed border-[rgba(15,23,42,0.15)] dark:border-[rgba(255,255,255,0.15)] bg-[rgba(15,23,42,0.02)] dark:bg-[rgba(255,255,255,0.02)]">
                    <p className="text-sm text-[rgba(11,18,32,0.4)] dark:text-slate-500 mb-3">
                        No hay registros de repuestos ni gastos adicionales en esta orden.
                    </p>
                </div>
            )}
