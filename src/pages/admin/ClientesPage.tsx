        ...
        <AdminLayout>
            <div className="space-y-5 animate-fade-in">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-[#0B1220] tracking-tight">Clientes</h1>
                        <p className="text-sm text-[rgba(11,18,32,0.50)] mt-0.5">{clientes.length} registrados</p>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {filtered.map((c, i) => (
                            <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                {c.telefono && (
                                    <div className="flex items-center gap-2 text-xs text-[rgba(11,18,32,0.55)]">
                                        <Phone className="w-3 h-3 text-[#FF5100]" /> {c.telefono}
                                    </div>
                                )}
                            </motion.div>
                        ))}
                        {filtered.length === 0 && (
                            <div className="sm:col-span-2 lg:col-span-3 card flex flex-col items-center py-14 gap-3">
                                <Users className="w-8 h-8 text-[rgba(11,18,32,0.20)]" />
                                <p className="text-sm text-[rgba(11,18,32,0.40)]">No se encontraron clientes</p>
                            </div>
                        )}
                    </div>
                )}
